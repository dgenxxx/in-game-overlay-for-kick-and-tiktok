...
class OverlayWindow extends JFrame {
    ...
    private JLabel kickViewerLabel;
    private JLabel tiktokViewerLabel;
    ...
    private ConcurrentHashMap<String, Long> kickViewers = new ConcurrentHashMap<>();
    private ConcurrentHashMap<String, Long> tiktokViewers = new ConcurrentHashMap<>();
    private int kickViewerCount = 0;
    private int tiktokViewerCount = 0;
    private static final long VIEWER_TIMEOUT_MS = 60 * 1000; // 1 minute
    private javax.swing.Timer viewerCleanupTimer;
    
    // Join notification panel
    private JPanel joinPanel;
    private JLabel joinLabel;
    private ConcurrentLinkedQueue<String> joinQueue = new ConcurrentLinkedQueue<>();
    private boolean isShowingJoin = false;
    private javax.swing.Timer joinDisplayTimer;
    ...
    public OverlayWindow(boolean soundEnabled, ConfigManager config) {
        ...
        // Start viewer cleanup timer (runs every 10 seconds for more frequent updates)
        viewerCleanupTimer = new javax.swing.Timer(10000, e -> cleanupInactiveViewers());
        viewerCleanupTimer.start();
        ...
        // Create viewer panel at the top
        viewerPanel = new JPanel();
        viewerPanel.setLayout(new BoxLayout(viewerPanel, BoxLayout.Y_AXIS));
        viewerPanel.setOpaque(false);
        viewerPanel.setBorder(BorderFactory.createEmptyBorder(5, 10, 10, 10));
        
        kickViewerLabel = new JLabel("Kick: 0 viewers");
        kickViewerLabel.setForeground(Color.GREEN);
        kickViewerLabel.setFont(new Font("Arial", Font.BOLD, 12));
        kickViewerLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
        
        tiktokViewerLabel = new JLabel("TikTok: 0 viewers");
        tiktokViewerLabel.setForeground(new Color(0, 255, 255));
        tiktokViewerLabel.setFont(new Font("Arial", Font.BOLD, 12));
        tiktokViewerLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
        
        viewerPanel.add(kickViewerLabel);
        viewerPanel.add(Box.createRigidArea(new Dimension(0, 3)));
        viewerPanel.add(tiktokViewerLabel);
        ...
        // Create join notification panel at the bottom
        joinPanel = new JPanel();
        joinPanel.setLayout(new FlowLayout(FlowLayout.LEFT, 10, 8));
        joinPanel.setOpaque(true); // Make it opaque so it's visible
        joinPanel.setBackground(new Color(30, 30, 30, 200)); // Dark semi-transparent background
        joinPanel.setBorder(BorderFactory.createEmptyBorder(8, 15, 8, 15));
        joinPanel.setVisible(false); // Hidden by default
        
        joinLabel = new JLabel();
        joinLabel.setForeground(new Color(255, 255, 255)); // White text for better visibility
        joinLabel.setFont(new Font("Arial", Font.BOLD, 14));
        
        joinPanel.add(joinLabel);
        contentPane.add(joinPanel, BorderLayout.SOUTH);
        
        System.out.println("OverlayWindow: Join panel created and added to SOUTH");
    ...
    public void addMessage(final ChatMessage msg) {
        System.out.println("OverlayWindow: addMessage() called!");
        System.out.println("OverlayWindow: Message: " + (msg != null ? msg.message : "NULL"));
        System.out.println("OverlayWindow: Platform: " + (msg != null ? msg.platform : "NULL"));
        System.out.println("OverlayWindow: Window visible: " + isVisible());
        System.out.println("OverlayWindow: Total messages before add: " + messages.size());
        
        messages.add(msg);
        if (messages.size() > 20) {
            // Keep only the latest 20 messages
            System.out.println("OverlayWindow: Message limit reached (" + messages.size() + "), keeping only latest 20");
            while (messages.size() > 20) {
                messages.remove(0);
            }
            System.out.println("OverlayWindow: Messages after cleanup: " + messages.size());
            
            // Rebuild the entire UI after removal
            ...
            // Just add the new message without rebuilding everything
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    System.out.println("OverlayWindow: EDT - Adding single message");
                    
                    // Remove the filler if it exists
                    Component[] components = messagePanel.getComponents();
                    int componentCount = components.length;
                    if (componentCount > 0) {
                        Component lastComponent = components[componentCount - 1];
                        if (lastComponent instanceof Box.Filler) {
                            messagePanel.remove(lastComponent);
                            componentCount--;
                        }
                    }
                    ...
                    gbc.gridx = 0;
                    gbc.gridy = componentCount; // Use actual component count, not message count
                    gbc.weightx = 1.0;
                    gbc.fill = GridBagConstraints.HORIZONTAL;
                    gbc.anchor = GridBagConstraints.NORTHWEST;
                    gbc.insets = new Insets(0, 0, 5, 0);
                    
                    JPanel msgComponent = createMessageComponent(msg);
                    messagePanel.add(msgComponent, gbc);
                    System.out.println("OverlayWindow: EDT - Added message at row " + gbc.gridy + ": " + msg.message + " (Platform: " + msg.platform + ")");
                    
                    // Re-add filler to push messages up
                    GridBagConstraints fillerGbc = new GridBagConstraints();
                    fillerGbc.gridx = 0;
                    fillerGbc.gridy = componentCount + 1;
                    fillerGbc.weighty = 1.0;
                    messagePanel.add(Box.createGlue(), fillerGbc);
                    
                    messagePanel.revalidate();
                    messagePanel.repaint();
                    System.out.println("OverlayWindow: EDT - UI update complete");
                }
            });
        }
        
        System.out.println("OverlayWindow: Total messages after add: " + messages.size());
        soundPlayer.playSound();
    }
    
    private void rebuildAllMessages() {
        System.out.println("OverlayWindow: EDT - Rebuilding all messages");
        ...
        for (ChatMessage m : messages) {
            JPanel msgComponent = createMessageComponent(m);
            messagePanel.add(msgComponent, gbc);
            gbc.gridy++;
        }
        ...
        messagePanel.revalidate();
        messagePanel.repaint();
        System.out.println("OverlayWindow: EDT - Rebuild complete");
    }
    
    private JPanel createMessageComponent(ChatMessage msg) {
        System.out.println("OverlayWindow: createMessageComponent() START for: " + msg.message);
        ...
        // Add platform icon on the left
        if (msg.platform != null && !msg.platform.equals("System")) {
            System.out.println("OverlayWindow: Loading icon for platform: " + msg.platform);
            JLabel iconLabel = new JLabel();
            iconLabel.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 5));
            if (msg.platform.equals("Kick")) {
                iconLabel.setIcon(PlatformIcons.getKickIcon());
                System.out.println("OverlayWindow: Kick icon loaded");
            } else if (msg.platform.equals("TikTok")) {
                iconLabel.setIcon(PlatformIcons.getTikTokIcon());
                System.out.println("OverlayWindow: TikTok icon loaded");
            }
        ...
        javax.swing.text.SimpleAttributeSet badgeStyle = new javax.swing.text.SimpleAttributeSet();
        javax.swing.text.StyleConstants.setBold(badgeStyle, true);
        javax.swing.text.StyleConstants.setFontFamily(badgeStyle, "Arial");
        javax.swing.text.StyleConstants.setFontSize(badgeStyle, 14);
        ...
        System.out.println("OverlayWindow: createMessageComponent() COMPLETE for: " + msg.message);
        return panel;
    }
    ...
    public void updateKickViewerCount(int count) {
        System.out.println("OverlayWindow: updateKickViewerCount called with: " + count);
        this.kickViewerCount = count;
        updateViewerDisplay();
    }
    
    public void updateTikTokViewerCount(int count) {
        System.out.println("OverlayWindow: updateTikTokViewerCount called with: " + count);
        System.out.println("OverlayWindow: Previous tiktokViewerCount: " + this.tiktokViewerCount);
        this.tiktokViewerCount = count;
        System.out.println("OverlayWindow: New tiktokViewerCount: " + this.tiktokViewerCount);
        updateViewerDisplay();
    }
    
    public void addKickViewer(String username) {
        if (username != null && !username.isEmpty()) {
            kickViewers.put(username, System.currentTimeMillis());
            System.out.println("OverlayWindow: Added Kick viewer: " + username + " (Total: " + kickViewers.size() + ")");
            updateViewerDisplay();
        }
    }
    
    public void removeKickViewer(String username) {
        kickViewers.remove(username);
        System.out.println("OverlayWindow: Removed Kick viewer: " + username);
        updateViewerDisplay();
    }
    
    public void addTikTokViewer(String username) {
        if (username != null && !username.isEmpty()) {
            tiktokViewers.put(username, System.currentTimeMillis());
            System.out.println("OverlayWindow: Added TikTok viewer: " + username + " (Total: " + tiktokViewers.size() + ")");
            updateViewerDisplay();
        }
    }
    
    public void removeTikTokViewer(String username) {
        tiktokViewers.remove(username);
        System.out.println("OverlayWindow: Removed TikTok viewer: " + username);
        updateViewerDisplay();
    }
    
    public boolean hasTikTokViewer(String username) {
        return tiktokViewers.containsKey(username);
    }
    
    public void showJoinNotification(String username) {
        // Add to queue
        joinQueue.offer(username);
        System.out.println("OverlayWindow: Added to join queue: " + username + " (Queue size: " + joinQueue.size() + ")");
        
        // Start processing queue if not already showing
        if (!isShowingJoin) {
            processNextJoinNotification();
        }
    }
    
    private void processNextJoinNotification() {
        String username = joinQueue.poll();
        if (username == null) {
            // Queue is empty
            isShowingJoin = false;
            SwingUtilities.invokeLater(new Runnable() {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                joinLabel.setText("➜ " + username + " joined");
                joinPanel.setVisible(true);
                joinPanel.revalidate();
                joinPanel.repaint();
                System.out.println("OverlayWindow: Join panel set visible for: " + username);
                System.out.println("OverlayWindow: Join panel visible: " + joinPanel.isVisible());
                System.out.println("OverlayWindow: Join panel bounds: " + joinPanel.getBounds());
            }
        });
        
        // Schedule next notification after 5 seconds
        if (joinDisplayTimer != null) {
            joinDisplayTimer.stop();
        }
        joinDisplayTimer = new javax.swing.Timer(5000, e -> {
            processNextJoinNotification();
        });
        joinDisplayTimer.setRepeats(false);
        joinDisplayTimer.start();
    }
    ...
    public void clearKickViewers() {
        kickViewers.clear();
        kickViewerCount = 0;
        updateViewerDisplay();
    }
    
    public void clearTikTokViewers() {
        tiktokViewers.clear();
        tiktokViewerCount = 0;
        updateViewerDisplay();
    }
    
    private void cleanupInactiveViewers() {
        long now = System.currentTimeMillis();
        boolean changed = false;
        
        // Remove Kick viewers who haven't been active
        Iterator<Map.Entry<String, Long>> kickIter = kickViewers.entrySet().iterator();
        while (kickIter.hasNext()) {
            Map.Entry<String, Long> entry = kickIter.next();
            if (now - entry.getValue() > VIEWER_TIMEOUT_MS) {
                System.out.println("OverlayWindow: Removing inactive Kick viewer: " + entry.getKey());
                kickIter.remove();
                changed = true;
            }
        }
        
        // Remove TikTok viewers who haven't been active
        Iterator<Map.Entry<String, Long>> tiktokIter = tiktokViewers.entrySet().iterator();
        while (tiktokIter.hasNext()) {
            Map.Entry<String, Long> entry = tiktokIter.next();
            if (now - entry.getValue() > VIEWER_TIMEOUT_MS) {
                System.out.println("OverlayWindow: Removing inactive TikTok viewer: " + entry.getKey());
                tiktokIter.remove();
                changed = true;
            }
        }
    ...
    private void updateViewerDisplay() {
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                // Use kickViewerCount if set (from API), otherwise use tracked count
                int kickCount = kickViewerCount > 0 ? kickViewerCount : kickViewers.size();
                int tiktokCount = tiktokViewerCount > 0 ? tiktokViewerCount : tiktokViewers.size();
                
                System.out.println("OverlayWindow: Updating viewer display - Kick: " + kickCount + ", TikTok: " + tiktokCount);
                
                // Simple count display without usernames
                String kickText = "👁 Kick: " + kickCount + " viewer" + (kickCount != 1 ? "s" : "");
                String tiktokText = "👁 TikTok: " + tiktokCount + " viewer" + (tiktokCount != 1 ? "s" : "");
                
                kickViewerLabel.setText(kickText);
                tiktokViewerLabel.setText(tiktokText);
                System.out.println("OverlayWindow: Viewer labels updated - Kick: " + kickCount + ", TikTok: " + tiktokCount);
            }
        });
    }
